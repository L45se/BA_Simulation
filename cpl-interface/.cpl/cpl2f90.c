...
void dnsdirect_10vetaTOuvw();
void dnsdirect_13computeflowrate(double lambda_);
struct freefunc fortranfield_f;FILE *fortranfield_; 
...
/*! save*/
 fprintf(stdout,"""Writing CPL field...""");putc('\n',stdout);
/*!time = dt_save*/
/*!outstats()*/
/*! save in Fortran format*/
 fprintf(stdout,"""Writing Fortran field...""");putc('\n',stdout);
fortranfield_=NULL;atblockexit(fortranfield_f,errfclose,&fortranfield_); errfopen(&fortranfield_,"Dati.cart.fort",O_RDWR|O_CREAT|O_TRUNC);fwrite(&nx_,(ssize_t)sizeof(int),1,fortranfield_);fwrite(&ny_,(ssize_t)sizeof(int),1,fortranfield_);fwrite(&nz_,(ssize_t)sizeof(int),1,fortranfield_);fwrite(&alfa0_,(ssize_t)sizeof(double),1,fortranfield_);fwrite(&beta0_,(ssize_t)sizeof(double),1,fortranfield_);fwrite(&ni_,(ssize_t)sizeof(double),1,fortranfield_);fwrite(&a_,(ssize_t)sizeof(double),1,fortranfield_);fwrite(&ymin_,(ssize_t)sizeof(double),1,fortranfield_);fwrite(&ymax_,(ssize_t)sizeof(double),1,fortranfield_);fwrite(&time_,(ssize_t)sizeof(double),1,fortranfield_);
cpl2f90_1h=nx_;
cpl2f90_2i=(ssize_t)sizeof(struct COMPLEX_)*((dnsdata_5nyh+2)-(dnsdata_4nyl-2)+1);
cpl2f90_3i=cpl2f90_2i*(dnsdata_49h-dnsdata_48l+1);
cpl2f90_7h=nx_;
cpl2f90_11h=nx_;
{ {int _4i_=0  ;do{{ { {int _5i_=dnsdata_48l  ;do{{ { {int _6i_=(dnsdata_4nyl-2)  ;do{{ fwrite((struct COMPLEX_*)(_6i_*(ssize_t)sizeof(struct COMPLEX_)*((((ssize_t)sizeof(struct COMPLEX_)+(ssize_t)sizeof(struct COMPLEX_)+(ssize_t)sizeof(struct COMPLEX_))/((ssize_t)sizeof(struct COMPLEX_))-1)/*SA37*/+1)+_5i_*dnsdata_50i+_4i_*dnsdata_51i+V_),(ssize_t)sizeof(struct COMPLEX_),1,fortranfield_);}_6i_+=1;}while(_6i_<=(dnsdata_5nyh+2));}}}_5i_+=1;}while(_5i_<=dnsdata_49h);}}}_4i_+=1;}while(_4i_<=cpl2f90_1h);}} { {int _8i_=0  ;do{{ { {int _9i_=dnsdata_48l  ;do{{ { {int _10i_=(dnsdata_4nyl-2)  ;do{{ fwrite((struct COMPLEX_*)(_10i_*(ssize_t)sizeof(struct COMPLEX_)*((((ssize_t)sizeof(struct COMPLEX_)+(ssize_t)sizeof(struct COMPLEX_)+(ssize_t)sizeof(struct COMPLEX_))/((ssize_t)sizeof(struct COMPLEX_))-1)/*SA37*/+1)+_9i_*dnsdata_50i+_8i_*dnsdata_51i+(ssize_t)sizeof(struct COMPLEX_)+V_),(ssize_t)sizeof(struct COMPLEX_),1,fortranfield_);}_10i_+=1;}while(_10i_<=(dnsdata_5nyh+2));}}}_9i_+=1;}while(_9i_<=dnsdata_49h);}}}_8i_+=1;}while(_8i_<=cpl2f90_7h);}} { {int _12i_=0  ;do{{ { {int _13i_=dnsdata_48l  ;do{{ { {int _14i_=(dnsdata_4nyl-2)  ;do{{ fwrite((struct COMPLEX_*)(_14i_*(ssize_t)sizeof(struct COMPLEX_)*((((ssize_t)sizeof(struct COMPLEX_)+(ssize_t)sizeof(struct COMPLEX_)+(ssize_t)sizeof(struct COMPLEX_))/((ssize_t)sizeof(struct COMPLEX_))-1)/*SA37*/+1)+_13i_*dnsdata_50i+_12i_*dnsdata_51i+(ssize_t)sizeof(struct COMPLEX_)+(ssize_t)sizeof(struct COMPLEX_)+V_),(ssize_t)sizeof(struct COMPLEX_),1,fortranfield_);}_14i_+=1;}while(_14i_<=(dnsdata_5nyh+2));}}}_13i_+=1;}while(_13i_<=dnsdata_49h);}}}_12i_+=1;}while(_12i_<=cpl2f90_11h);}}
errfclose(&fortranfield_);
